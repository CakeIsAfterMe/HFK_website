...
{{< lead >}}
Конспект посвящён модулям `os` и `shutil`, которые могут быть полезны для работы с операционной системой, управлением каталогами и файлами, а также модулю `shelve`, позволяющий сериализовывать объекты Python
{{< /lead >}}
